...
class RuffChecker(StyleChecker):
    patterns = ("*.py",)
    results_regex = re.compile(r"^.+:(\d+):(\d+): (.+)$")
    ...
    def check(self, line_numbers):
        ...
        try:
            ...
            issues.append(
                StyleIssue(0, None, "Please install ruff to validate python additions")
            )
            return issues

        results = ret.stdout.decode("utf-8").splitlines()
        for item in results:
            search = re.search(RuffChecker.results_regex, item)
            if not search:
                continue
