...
static gboolean parse_output_caps(GstPispConvert *self, guint index, GstCaps *caps)
{
	...
	/* Default output dimensions and colorspace to match input when unspecified */
	if (!self->priv->out_width[index])
		self->priv->out_width[index] = self->priv->in_width;
	if (!self->priv->out_height[index])
		self->priv->out_height[index] = self->priv->in_height;
...
static gboolean gst_pisp_convert_configure(GstPispConvert *self)
{
	...
	try
	{
		...
		for (unsigned int i = 0; i < PISP_NUM_OUTPUTS; i++)
		{
			...
			GST_INFO_OBJECT(self, "Crop%u: offset=(%u,%u) size=%ux%u", i, crop.offset_x, crop.offset_y, crop.width,
							crop.height);
